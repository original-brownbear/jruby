...
public class JavaInterfaceBenchmark {
    ...
    @Benchmark
    @OperationsPerInvocation(INVOCATIONS)
    public void benchFullRubyVersion(final Blackhole blackhole) {
        blackhole.consume(
            RUBY.evalScriptlet(
                String.format(
                    "summer = RubySummation.new\n%d.times {|_| res = summer.sum(%d, %d)}",
                    INVOCATIONS, LEFT.getLongValue(), RIGHT.getLongValue()
                )
            )
        );
    }
